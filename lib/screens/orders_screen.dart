...
import 'order_webview_screen.dart';
...
class _OrdersScreenState extends State<OrdersScreen>
  ...
  Future<void> _refreshOrders() async {
    ...
    try {
      await Future.delayed(const Duration(
          milliseconds: 500)); // Minimum loading time for better UX
      setState(() {
        _ordersFuture = _loadOrdersAndTrackChanges();
      });
  ...
  Future<void> _trackOrderStatusChanges(List<Order> orders) async {
    ...
    final Map<String, String> oldStatuses =
        Map<String, String>.from(json.decode(storedStatuses));
    ...
    for (final order in orders) {
      ...
      if (previousStatus != null && previousStatus != currentStatus) {
        ...
        final notification = {
          'title':
              langCode == 'ar' ? 'تحديث حالة الطلب' : 'Order Status Update',
          'body': langCode == 'ar'
        ...
        itemBuilder: (context, index) {
          ...
          return AnimatedContainer(
            ...
            child: FilterChip(
              ...
              label: Row(
                ...
                children: [
                  ...
                  Text(
                    ...
                    style: TextStyle(
                      color:
                          isSelected ? Colors.white : const Color(0xFF1A2543),
                      fontWeight:
              ...
              side: BorderSide(
                color:
                    isSelected ? const Color(0xFF6FE0DA) : Colors.grey.shade300,
              ),
          ...
          child: Opacity(
            ...
            child: Container(
              ...
              child: Material(
                ...
                child: InkWell(
                  borderRadius: BorderRadius.circular(16),
                  onTap: () => _openOrderDetailsInWebView(order, langCode),
                  child: Padding(
                    padding: const EdgeInsets.all(16),
                    child: Column(
                      children: [
    ...
    if (confirmed == true) {
      ...
      try {
        ...
        if (success) {
          ...
          throw Exception(
              isAr ? 'فشل في إلغاء الطلب' : 'Failed to cancel order');
        }
      } catch (e) {
        Navigator.pop(context); // Close loading
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  void _openOrderDetailsInWebView(Order order, String langCode) {
    final token =
        (order.metaData['customer_view_token'] ?? '').toString().trim();
    final isAr = langCode == 'ar';
          ...
          content: Row(
            children: [
              ...
              Expanded(
                child: Text(
                  isAr
                      ? 'لم يتم تجهيز خطة التقسيط لهذا الطلب بعد.'
                      : 'The installment plan for this order is not ready yet.',
                ),
              ),
            ],
          ),
    ...
    final url =
        Uri.parse('${AppConfig.adminBaseUrl}/public_order_detail.php').replace(
      queryParameters: {
